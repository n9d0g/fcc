...
export const PraiseSchedule = (props: any) => {
  ...
  const originalPraiseData = props.data
		.filter((praiseData: any) =>
			praiseData.date > today.toISOString().split('T')[0])
		.sort((a: any, b: any) =>
    	a.date > b.date ? 1 : -1
		)
		...
		<ScheduleContainer>
			...
			<WeekContainer>
				<WeekDataHeaderContainer>
					<WeekDataHeader>date</WeekDataHeader>
					<WeekDataHeader>leader 😎</WeekDataHeader>
					<WeekDataHeader>guitar 🎸</WeekDataHeader>
					<WeekDataHeader>piano 🎹</WeekDataHeader>
					<WeekDataHeader>bass 🤘</WeekDataHeader>
					<WeekDataHeader>drums 🪘</WeekDataHeader>
					<WeekDataHeader>backup 🎤</WeekDataHeader>
					<WeekDataHeader>AV 🔊</WeekDataHeader>
					<WeekDataHeader>info 📖</WeekDataHeader>
				</WeekDataHeaderContainer>
					...
					return (
						<Fragment key={index}>
							...
							<WeekData first={first}>
								<InfoModal topic={item.topic} scripture={item.scripture} summary={item.summary} />
							</WeekData>
						</Fragment>
					)
				})}
...
const WeekData = styled.p<WeekDataProps>`
  ...
  border: 1px solid var(--secondary-grey);
	color: ${props => props.first ? `red` : `black`};
