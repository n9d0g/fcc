...
const sgPage: string = 'https://dev.fcccanada.ca/small-groups'
const YAPage: string = 'https://dev.fcccanada.ca/small-groups/young-adults'
...
test('smallgroups: has main header', async ({ page }) => {
	await page.goto(sgPage)
	await expect(page.getByRole('heading', { name: 'Our Small Groups.' })).toBeVisible()
})
...
test('smallgroups/YA: YA men button visible', async ({ page }) => {
	await page.goto(YAPage)
	await expect(page.getByRole('link', { name: 'YA Men', exact: true })).toBeVisible()
})
